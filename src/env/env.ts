// const vars = {
//   // hato_fire_base_auth_key: `AIzaSyCY2w-6FsuqmORzke7J6anzvsFIrjnXTIs`,
//   //no_auth:true,
//   no_auth: false,
//   firebase_config: {
//     apiKey: "AIzaSyDqoX52nqdqjFFZW6dJgo48dR2MW8y7Ep8",
//     databaseURL:
//       "https://baolondon-ae6db-default-rtdb.europe-west1.firebasedatabase.app",
//     authDomain: "baolondon-ae6db.firebaseapp.com",
//     projectId: "baolondon-ae6db",
//     storageBucket: "baolondon-ae6db.appspot.com",
//     messagingSenderId: "175175266110",
//     appId: "1:175175266110:web:0fb5160bf044d6d47e866f",
//     measurementId: "G-1SHEXEH5LP",
//   },
// };
// export default vars;



const vars = {
  // hato_fire_base_auth_key: `AIzaSyCY2w-6FsuqmORzke7J6anzvsFIrjnXTIs`,
  //no_auth:true,
  no_auth: false,
  firebase_config: {
    apiKey: "AIzaSyDqoX52nqdqjFFZW6dJgo48dR2MW8y7Ep8",
    databaseURL:
      "https://baolondon-ae6db-default-rtdb.europe-west1.firebasedatabase.app",
    authDomain: "baolondon-ae6db.firebaseapp.com",
    projectId: "baolondon-ae6db",
    storageBucket: "baolondon-ae6db.appspot.com",
    messagingSenderId: "175175266110",
    appId: "1:175175266110:web:ced5efaa6d4a9e2d7e866f",
    measurementId: "G-PZ6WEKHTV7",
  },
};
export default vars;
